...
from model.model_ner.bert_lstm_crf import *
...
#     use_cuda = False
vocab = load_vocab('model/medical_ner/vocab.txt')
...
def evaluate(medel, dev_loader):
    ...
    with torch.no_grad():
        ...
        acc,p, r, f = get_f1(gold_label_1,pred_label_1)
...
best_f = -100
model_name = 'model/medical_ner' + '0518' + str(float('%.3f' % best_f)) + ".pkl"
print(model_name)
...
for epoch in range(epochs):
    ...
    if f > best_f:
        ...
        model_name = 'model/medical_ner' + 'new' + str(float('%.3f' % best_f)) + ".pkl"
